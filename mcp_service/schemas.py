...
class RunModelRequest(MCPModel):
    ...
    task_name: str = Field(
        ...
        max_length=256,
        description=(
            "Optional human-readable task label. The MCP service generates the "
            "unique downstream task name used in output file names."
        ),
    )
...
class RunModelResult(MCPModel):
    ...
    task_name: NonEmptyId = Field(
        description="Unique task name generated by the MCP service."
    )
    resolved_outputs: dict[str, str] = Field(
        default_factory=dict,
        description="Server-resolved output paths submitted to the model service.",
    )
